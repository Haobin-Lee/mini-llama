...
size_t checkedNumel(const std::vector<int>& shape, const char* caller) {
    size_t total = 1;
    if (shape.empty()) {
        throw std::logic_error(std::string(callerName(caller)) +
                               ": shape is empty");
    }
    ...
    for (int axis = 0; axis < shape.size(); ++axis) {
        ...
        if (dim <= 0) {
            throw std::logic_error(
                std::string(callerName(caller)) + ": dimension at axis " +
                std::to_string(axis) + " must be positive, got " +
                std::to_string(dim) + " in shape " + shapeToString(shape));
        }
...
void checkRank(const Tensor& t, int expected_rank, const char* caller) {
    if (t.numDims() != expected_rank) {
        throw std::logic_error(std::string(callerName(caller)) + " expected " +
                               std::to_string(expected_rank) +
                               "D Tensor, got " + t.shapeString());
    }
}
...
void checkAxisIndex(const Tensor& t, int axis, int flat_index,
    ...
    if (flat_index < 0 || flat_index >= dim) {
        throw std::out_of_range(
            std::string(callerName(caller)) + ": FlatIndex " +
            std::to_string(flat_index) + " out of range for axis " +
            std::to_string(axis) + " with size " + std::to_string(dim) +
            " in tensor " + t.shapeString());
    }
}
}  // namespace
...
size_t Tensor::flatIndex(const std::vector<int>& indices) const {
    if (indices.size() != shape.size()) {
        throw std::runtime_error("Tensor::FlatIndex expected " +
                                 ...
                                 " indices for tensor " + shapeString() +
...
void Tensor::assertShape(const std::vector<int>& expected,
    ...
    if (shape != expected) {
        throw std::runtime_error(
            ...
            shapeToString(expected) + ", got " + shapeString());
    }
}
...
void Tensor::print(const std::string& name, bool print_data) const {
    ...
    std::cout << "shape = " << shapeString() << " size = " << size()
              << std::endl;
    if (print_data) {
        std::cout << "data = ";
